...
float3 GetTotalScatteringLerped(float r, float mu, float mu_s, float nu, bool ray_r_mu_intersects_ground) {
	...
	return
		...
		+ InternalGetMultipleLerped(atm, r, mu, mu_s, nu, ray_r_mu_intersects_ground)) * _SunIrradianceOnAtm;
}
...
float3 EvaluateSunDiskRadianceOfUnitRadiance(AtmosphereParameters atm, float r, float mu, float nu) {
	...
	return _SunIrradianceOnAtm * radianceTransmitted / solidAngle / 500.0f;	//Avoiding it blind your eye.
}
...
float3 GetTransmittanceWithCameraVolume(float3 uvw) {
	return tex3Dlod(_CameraVolumeTransmittance, float4(uvw, 0.0f)).rgb * _SunIrradianceOnAtm;
}

float3 GetScatteringWithCameraVolume(float3 uvw) {
	return tex3Dlod(_CameraVolumeScattering, float4(uvw, 0.0f)).rgb * _SunIrradianceOnAtm;
}
