...
Shader "Yangrc/CloudShader"
{
	Properties
	{
		...
		_AtmosphereColorSaturateDistance("AtmosphereColorSaturateDistance", float) = 80000
			...
			_CloudShadowStrength("Cloud shadow strength", float) = 0.4
	}
		...
		//Pass3, Calculate lighting, blend with final image.
		Pass{
			Cull Off ZWrite Off ZTest Always
			CGPROGRAM
			#pragma vertex vert
...
#include "BilateralUpsampleHelper.cginc"
		...
		Pass{
			...
			float frag(v2f i) : SV_Target
			{
				...
				//Using min operator causes some "ghosting" effects, due to we used temporal info. A max operator doesn't solve it, but hides it better for most situations.
		...
		ENDCG
			...
			//Pass6, evaluate screen space cloud shadow.
		Pass{
			Cull Off ZWrite Off ZTest Always
			CGPROGRAM
			#include "CloudShadowHelper.cginc"
			#pragma vertex VertCloudShadow
			#pragma fragment FragCloudShadow
			ENDCG
		}

		//Pass7, Combine cloud shadow with screen space shadow..
		Pass{
			Cull Off ZWrite Off ZTest Always
			Blend Zero SrcAlpha
			CGPROGRAM
			...
			float _CloudShadowStrength = 0.2f;
			sampler2D _ScreenSpaceShadowMask;
			sampler2D _MainTex;
			struct appdata
			{
				float4 vertex : POSITION;
			...
			float4 frag(v2f i) : SV_Target
			{
				float cloudShadow = tex2D(_MainTex, i.uv).r;	//1.0 for darkest, 0.0 for not occluded.
				cloudShadow *= _CloudShadowStrength;
				return 1.0f - cloudShadow;
			}
			ENDCG
		}
	}
