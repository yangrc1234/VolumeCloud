...
float SampleHeight(float heightPercent,float cloudType) {
	...
	float cumulus = RemapClamped(heightPercent, 0.0, .3, 0.0, 1.0) * RemapClamped(heightPercent, .4, 0.9, 1.0, 0.0);
...
float Inscatter(float3 worldPos,float dl, float cosTheta) {
	...
	depth_probability = lerp(depth_probability, 1.0, saturate(dl * 50));		//I think the original one in slider is wrong.(or they use dl as "brigtness" rather than "occlusion"
...
float Energy(float3 worldPos, float d, float cosTheta) {
	...
	return Inscatter(worldPos, d, cosTheta) *hgImproved * BeerLaw(d, cosTheta) * 2.5;
}
...
float GetDentisy(float3 startPos, float3 dir,float maxSampleDistance, float raymarchOffset, out float intensity,out float depth) {
	...
	*/
	...
	float earthRadius = 640000;		//Earth is scale down by 10. (I think this looks better.)
	...
	int sample_count = lerp(128, MIN_SAMPLE_COUNT, dir.y);	//dir.y ==0 means horizontal, use maximum sample count
	
	//When near horizon, we enlarge the "cheap step multiplier"
	float largeSampleStepSize = 2 / (saturate(dir.y + 0.5) + 0.01);
	...
	float sample_step = lerp(MAX_SAMPLE_COUNT, MIN_SAMPLE_COUNT, dir.y);
	...
	for (int j = 0; j < sample_count; j++) {
		...
		if (!detailedSample) {
			...
			if (sampleResult > 0) {
				detailedSample = true;
				raymarchDistance -= sample_step * largeSampleStepSize;
				missedStepCount = 0;
				continue;
			}
			else {
			...
			if (sampleResult <= 0) {
				...
				float sampledAlpha = sampleResult * sample_step * _CloudDentisy;	
